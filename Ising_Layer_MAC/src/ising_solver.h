...
// s_i: {-1, 1}
class IsingSolver {
  int steps, total_step, seq_clust;
...
public:
  ...
  IsingSolver(const CostFunction& cf, const int opt_size);
  ...
  void step();
  void init(const InitMode mode, const double cool_coe, const double update_ratio, const double initial_active_ratio, const int seq_clust);
  void init(const InitMode mode, const int seed, const double cool_coe, const double update_ratio, const double initial_active_ratio, const int seq_clust);
  size_t getActiveNodeCount() const;
  size_t getUpdateNodeCount() const;
  size_t size() const;
  size_t map_size() const;
  size_t opt_size() const;
  size_t size_opt() const;
  int getStep() const;
