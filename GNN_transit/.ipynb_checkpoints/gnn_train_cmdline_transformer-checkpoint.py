...
from torch_geometric.nn import TransformerConv  # TransformerConv from PyTorch Geometric
...
class GoogleTSPReader:
    ...
    def process_batch(self, lines):
        ...
        for b, line in enumerate(lines):
            ...
            # Generate edge_index and edge_attr for TransformerConv
            for i in range(self.num_nodes):
                for j in range(self.num_nodes):
                    if W[i, j] > 0:
                        # Global node indexing across the entire batch
                        global_i = b * self.num_nodes + i
                        global_j = b * self.num_nodes + j
                        edge_index_list.append([global_i, global_j])
                        edge_attr_list.append(int(W[i, j]))

        # Convert lists to tensors
        if len(edge_index_list) == 0:
            edge_index = torch.empty((2, 0), dtype=torch.long, device=device)  # (2, E_total)
            edge_attr = torch.empty((0,), dtype=torch.long, device=device)     # (E_total)
        else:
            edge_index = torch.tensor(edge_index_list, dtype=torch.long).t().contiguous()  # (2, E_total)
            edge_attr = torch.tensor(edge_attr_list, dtype=torch.long)  # (E_total)
        ...
        return DotDict(
            ...
            x_mid=batch_x_mid,
            y_mid=batch_y_mid,
            edge_index=edge_index,   # Added edge_index
...
class ResidualGatedGCNModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Node coordinate embedding using TransformerConv
        self.nodes_coord_embedding = TransformerConv(config.node_dim, self.hidden_dim, heads=1, concat=False)

        # Edge value and edge type embedding for GCN layers
        self.edges_values_embedding = nn.Linear(1, self.hidden_dim // 2, bias=False)
        self.edges_embedding = nn.Embedding(config.voc_edges_in, self.hidden_dim // 2)

        # Separate Edge Embedding for TransformerConv
        self.transformer_edge_embedding = nn.Embedding(config.voc_edges_in, self.hidden_dim)
    ...
    def forward(self, x_edges, x_edges_values, x_nodes_coord, edge_index, edge_attr):
        """
        Forward pass.

        Args:
            x_edges (torch.Tensor): Edge types, shape (B, V, V)
            x_edges_values (torch.Tensor): Edge values, shape (B, V, V)
            x_nodes_coord (torch.Tensor): Node coordinates, shape (B, V, 2)
            edge_index (torch.Tensor): Edge indices, shape (2, E_total)
            edge_attr (torch.Tensor): Edge attributes/types, shape (E_total,)
        
        Returns:
            torch.Tensor: Predicted edge logits, shape (B, V, V, O)
        """
        B, V, _ = x_nodes_coord.shape

        # Reshape node coordinates for TransformerConv
        # TransformerConv expects node features of shape (N, in_channels)
        # where N = B * V
        x_nodes_coord_flat = x_nodes_coord.view(B * V, -1)  # (N, node_dim)

        # Embed edge attributes for TransformerConv
        edge_attr_embedded = self.transformer_edge_embedding(edge_attr)  # (E_total, hidden_dim)

        # Apply TransformerConv
        x = self.nodes_coord_embedding(x_nodes_coord_flat, edge_index, edge_attr_embedded)  # (N, hidden_dim)

        # Reshape back to (B, V, H)
        x = x.view(B, V, self.hidden_dim)  # (B, V, H)
...
def plot_tsp_ground_truth_prediction_classifier(nodes_coord, edges_target, edges_pred_probs, edges_classified, x_mid, y_mid, title="TSP Visualization"):
    ...
    lines = [[nodes_coord[i], nodes_coord[j]] for i in range(num_nodes) for j in range(num_nodes) if edges_target[i, j] == 1]
    if lines:
    ...
    ax.scatter(nodes_coord[:, 0], nodes_coord[:, 1], color='blue', s=50, zorder=5)
    lines = [[nodes_coord[i], nodes_coord[j]] for i in range(num_nodes) for j in range(num_nodes) if edges_classified[i, j] == 1 and i != j]
    if lines:
...
def classify_edges(probs, threshold=0.95, max_edges=2):
    """
...
def train_one_epoch(net, optimizer, config, writer, epoch, logger=None):
    ...
    for batch_num in progress_bar:
        ...
        edge_index = batch.edge_index.to(device)   # (2, E_total)
        edge_attr = batch.edge_attr.to(device)     # (E_total,)
        ...
        # Forward pass
        y_pred_edges = net(x_edges, x_edges_values, x_nodes_coord, edge_index, edge_attr)
        ...
        y_classified = classify_edges(y_pred_probs, threshold=0.95, max_edges=2)  # (B, V, V)
    ...
    print(f"Epoch: {epoch}, Train Loss: {avg_loss:.4f}, Precision: {avg_precision:.4f}, Recall: {avg_recall:.4f}")
...
def evaluate(net, config, writer, epoch, mode='test'):
    ...
    print(f"Epoch: {epoch}, {mode.capitalize()} Loss: {avg_loss:.4f}, Precision: {avg_precision:.4f}, Recall: {avg_recall:.4f}")
...
def main():
    ...
    writer = SummaryWriter(log_dir='runs/gnn_training_transformer')
    ...
    config = DotDict(
        ...
        node_dim=2,
        voc_edges_in=3,      # Edge types: 0 (No Edge), 1 (Neighbor Edge), 2 (Self-Loop)
        voc_edges_out=2,     # Output classes: e.g., 0 (No Edge), 1 (Edge)
        hidden_dim=300,
        num_layers=30,
        mlp_layers=3,
        aggregation='mean',
    ...
    # **Removed DataParallel to prevent edge_index issues**
    # If you wish to use multiple GPUs, consider using DistributedDataParallel
